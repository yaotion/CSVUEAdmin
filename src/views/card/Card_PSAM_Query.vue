  ...
  <div class="app-container">
    ...
    <div id="tablePrint" ref="print">
      ...
      >
        ...
        <el-table-column label="制卡时间" prop="MakeTime" align="center" width="160" />
        <el-table-column label="卡号" prop="CardNo" align="center" width="160" />
        <el-table-column label="有效期" prop="BeginDate" align="center" width="180">
          <template slot-scope="scope2">
            <span>{{ scope2.row.BeginDate +'--'+ scope2.row.EndDate }}</span>
          </template>
        </el-table-column>
        <el-table-column label="终端编号" prop="TermNo" align="center" width="120" />
        <el-table-column label="重置卡号" prop="RemakeNum" align="center" width="140" />
  ...
  methods: {
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['序号', '发卡点', '操作员', '操作员编号', '制卡时间', '卡号', '起始有效日期', '终止有效日期', '终端机编号', '重置卡号']
        const filterVal = ['index', 'stationno', 'OptName', 'OptNo', 'MakeTime', 'CardNo', 'BeginDate', 'EndDate', 'TermNo', 'RemarkNum']
